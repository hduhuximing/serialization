...
public class SessionTest extends BaseTestSupport
{
    ...
    // List<Object[]> list = (List<Object[]>) session.listQuery(new Class<?>[] {
    // Integer.class, String.class, Integer.class }, sql, 1);
    ...
    // {
    // logger.info("测试sqlQueryByName(Class<User> t, String sql, Object...
    // params)");
    ...
    // session.getConnection().prepareStatement("select * from user where
    // userid=1");
    ...
    // {
    // logger.info("测试{}", "batchSqlUpdate(String sql, Iterator<Object[]>
    // iterator)");
    ...
    // list.add(data);
    // String sql = "insert into user (username,birthday,age,userid,password)
    // values(?,?,?,?,?)";
    // session.batchUpdate(sql, list);
    // try
    // {
    // PreparedStatement pstat =
    // session.getConnection().prepareStatement("select * from user where userid
    // in(5,4) order by userid");
    ...
    // session.update("insert into user (username,age,userid,password)
    // values('你好',12,4,'sdads')");
    // SqlSession tmp = sessionFactory.openSession();
    // List<Object[]> tmpResult = (List<Object[]>) tmp.listQuery(new Class<?>[]
    // { String.class, Integer.class }, "select username,age from user where
    // userid=4");
