...
public class ReadWriteExecutor implements EventExecutor
{
    ...
    private final MPSCQueue<ReadWriteCoordinatorContext<?>> queue         = new MPSCQueue<ReadWriteCoordinatorContext<?>>();
    
    @Override
    public void handle(CoordinatorContext<?> eventContext, RunnerMode runnerMode)
    {
        ReadWriteCoordinatorContext<?> readWriteEventContext = (ReadWriteCoordinatorContext<?>) eventContext;
        if (readWriteEventContext.mode() == ReadWriteCoordinatorContext.READ)
        {
            if (readWriteEventContext.immediateInvoke() == false)
            {
                if (tryFastInvokeRead(readWriteEventContext, runnerMode) == false)
                {
                    fullPathInvokeRead(readWriteEventContext, runnerMode);
                }
            }
            else
            {
                CoordinatorHelper.handle(readWriteEventContext, runnerMode);
                int now = readWriteLock.releaseReadLock();
                if (readLocks(now) == 0 && queueLocks(now))
                {
                    handleQueue(runnerMode);
                }
                else
                {
                    ;
    ...
    private boolean tryFastInvokeRead(ReadWriteCoordinatorContext<?> readWriteEventContext, RunnerMode runnerMode)
    {
    ...
    private void fullPathInvokeRead(ReadWriteCoordinatorContext<?> readWriteEventContext, RunnerMode runnerMode)
    {
        int now;
    ...
    private void handleQueue(RunnerMode runnerMode)
    {
        ReadWriteCoordinatorContext<?> pollEvent;
        while ((pollEvent = queue.peek()) != null)
        {
            if (pollEvent.mode() == ReadWriteCoordinatorContext.WRITE)
            {
                pollEvent = queue.poll();
                CoordinatorHelper.handle(pollEvent, runnerMode);
            }
            else
            {
                List<CoordinatorContext<?>> list = new LinkedList<CoordinatorContext<?>>();
                while ((pollEvent = queue.peek()) != null)
                {
                    if (pollEvent.mode() == ReadWriteCoordinatorContext.READ)
                    {
                        pollEvent = queue.poll();
                ...
                for (CoordinatorContext<?> each : list)
                {
                    CoordinatorBuses.computation().post(each);
                }
